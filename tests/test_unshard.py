...
import numpy as np
from jax_progress.unshard import unshard_size, unshard_min, unshard_max, unshard_iota
...
@pytest.mark.parametrize("mesh_shape,axis_names,spec,expected_size", [
...
@pytest.mark.parametrize("mesh_shape,axis_names,spec", [
...
def test_unshard_min_with_shard_map(mesh_shape, axis_names, spec):
    ...
    data_shape = tuple([mesh_shape[i] if spec[i] is not None else 1
